...
export function applySurfaces(THREE, root, opts = {}) {
  ...
  root.traverse((o) => {
    ...
    // ATLAS TEXTURES WIN. The level now assigns real generated maps to its
    // materials before this runs, and this function's whole job is to REPLACE a
    // material's maps with procedural ones — so without this guard it silently
    // discarded every Atlas texture and the road rendered as flat tan even
    // though the material I authored was near-black. A material that already
    // carries a map is already surfaced; leave it alone.
