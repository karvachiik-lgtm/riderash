async (page) => {
  ...
  try {
    ...
    res = await p.evaluate(()=>{
      const sel = ['.tp','.hb','#hud .hud-tl','#hud .hud-tr','#hud .hud-br','#boxes','#healthlab','#stamlab','#combobar','#radar','#target'];
      const items=[];
      ...
      for (let i=0;i<items.length;i++){ const a=items[i].r;
        ...
        if (items[i].btn && (a.w<40||a.h<26)) small.push(items[i].n+' '+Math.round(a.w)+'x'+Math.round(a.h));
